...
if not isfolder("MinHook/Fonts") then
    makefolder("MinHook/Fonts")
end
local fontPath = "MinHook/Fonts/ProggyTiny.ttf"
if not isfile(fontPath) then
    writefile(fontPath, game:HttpGet("https://github.com/Chooseanewuser/PublicCode/raw/refs/heads/main/ProggyTiny.ttf"))
end
local round = function(number, float)
    return float * math.floor(number / float)
end
function Library.Fonts.Menu:Register_Font(Name, Weight, Style, Asset)
    local assetPath = "MinHook/Fonts/" .. Asset.Id
    if not isfile(assetPath) then writefile(assetPath, Asset.Font) end
...
getgenv().renderDistance = getgenv().renderDistance or 100000
getgenv().boxEnabled = false
getgenv().boxTransparency = getgenv().boxTransparency or 1
getgenv().boxGradient = getgenv().boxGradient or false
getgenv().BoxColor = Color3.fromRGB(255,255,255)
getgenv().boxOutlineEnabled = false
getgenv().boxOutlineTransparency = getgenv().boxTransparency or 1
getgenv().boxOutlineColor = getgenv().boxOutlineColor or Color3.fromRGB(0,0,0)
getgenv().nameEnabled = getgenv().nameEnabled or true
getgenv().nameColor = getgenv().nameColor or Color3.fromRGB(255,255,255)
getgenv().weaponEnabled = getgenv().weaponEnabled or true
getgenv().weaponESPColor = getgenv().weaponESPColor or Color3.fromRGB(255,255,255)
getgenv().healthBarEnabled = getgenv().healthBarEnabled or true
getgenv().healthBarOutlineEnabled = getgenv().healthBarOutlineEnabled or true
getgenv().distanceEnabled = getgenv().distanceEnabled or true
getgenv().distanceColor = getgenv().distanceColor or Color3.fromRGB(255,255,255)
local connections = {}
function isvisible(player)
    local localChar = LPlayer.Character
...
if not coreGui:FindFirstChild("Holder") then
    local Holder = cloneref(Instance.new("ScreenGui"))
    Holder.Parent = coreGui
    Holder.Name = "Holder"
end
function checkfordupes(plr)
    local holder = coreGui:FindFirstChild("Holder")
    if holder and holder:FindFirstChild(plr.Name) then
        holder[plr.Name]:Destroy()
    end
end
function destroyESP(plr)
    if coreGui:FindFirstChild("Holder") and coreGui.Holder:FindFirstChild(plr.Name) then
        for _, v in pairs(coreGui.Holder[plr.Name]:GetChildren()) do
            v:Destroy()
        end
        if connections[plr] then
            connections[plr].plrHoldster:Destroy()
            connections[plr] = nil
        end
    end
end
function addplr(plr)
    ...
    connections[plr] = {
        chams = {},
        plrHoldster = cloneref(Instance.new("Frame")),
        NameEsp = cloneref(Instance.new("TextLabel")),
        WeaponEsp = cloneref(Instance.new("TextLabel")),
        boxEsp = cloneref(Instance.new("Frame")),
        boxEspGradient = cloneref(Instance.new("UIGradient")),
        boxOutline = cloneref(Instance.new("UIStroke")),
        boxEspOutline = cloneref(Instance.new("Frame")),
        boxEspOutlineStroke = cloneref(Instance.new("UIStroke")),
        healthbarOutline = cloneref(Instance.new("Frame")),
        healthBar = cloneref(Instance.new("Frame")),
        healthBarGradient = cloneref(Instance.new("UIGradient")),
        healthbarStroke = cloneref(Instance.new("UIStroke")),
        ...
        if player ~= LPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if humanoidrootpart then
                ...
                if onScreen and distance < getgenv().renderDistance then
                    ...
                    data.NameEsp.Visible = false
                    data.boxEsp.Visible = false
                    data.boxEspOutline.Visible = false
                    data.WeaponEsp.Visible = false
